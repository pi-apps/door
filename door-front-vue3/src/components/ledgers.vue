<template>
	<div class="px-lg-4 pt-5 text-center border-bottom">
		<h1 class="display-5 fw-bold">Ledger Performance</h1>
...
const ledger_series = computed(() => {
	...
	return [
		{
			name: 'closing time',
			data: _(data).map('avg_ledger_time').value(),
			type: 'area',
		},
		{
			name: 'peocessed operations',
			data: _(data).map('operations').value(),
			type: 'column',
		},
	]
})
