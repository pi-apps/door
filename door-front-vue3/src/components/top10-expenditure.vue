...
<template>
  <div class="px-lg-4 pt-5 my-5 text-center border-bottom">
    <h1 class="display-5 fw-bold">Top 10 expenditure</h1>
...
const {
  data,
} = useRequest(() => api.get_top100_expenditure());
  ...
  return {
    ...
    dataLabels: {
      ...
      formatter: function (val, opt) {
        return "G..." + (opt.w.globals.labels[opt.dataPointIndex] + "").slice(52, 56) + ":  " + val + "pi"
      },
      offsetX: 0,
      dropShadow: {
        enabled: true
    ...
    title: {
      text: 'Top 10 expenditure address',
      align: 'center',
      floating: true
    },
    subtitle: {
...
const series = computed(() => {
  return [{
    name: 'txs',
    data: _.map(data?.value, 'expenditure')
  }]
})
